...
public class MmsSmsDatabaseHelper extends SQLiteOpenHelper {
    ...
    private MmsSmsDatabaseHelper(Context context) {
        super(context, DATABASE_NAME, null, determineDatabaseVersion(context));
...
//                   "END;");
        ...
        if (simCursor != null) {
            ...
            while (!simCursor.isAfterLast()) {
                ...
                db.execSQL("UPDATE " + MmsProvider.TABLE_PDU + " SET " + Mms.SUBSCRIPTION_ID
                        + " = " + subId + " WHERE " + Mms.PHONE_ID + " = " + phoneId);
                db.execSQL("UPDATE " + SmsProvider.TABLE_SMS + " SET " + Sms.SUBSCRIPTION_ID
                        + " = " + subId + " WHERE " + Sms.PHONE_ID + " = " + phoneId);
    ...
    private static int determineDatabaseVersion(Context context) {
        int count = 0;
        Cursor simCursor = context.getContentResolver().query(SubscriptionManager.CONTENT_URI,
                new String[] { SubscriptionManager.UNIQUE_KEY_SUBSCRIPTION_ID }, null, null, null);
        if (simCursor != null) {
            count = simCursor.getCount();
            simCursor.close();
        }
        return count > 0 ? DATABASE_VERSION : 63;
    }
}
